...
export default function AdminDashboard() {
  ...
  // Estados locales para los datos del Backend
  const [totalLibros, setTotalLibros] = useState(0);
  const [totalUsuarios, setTotalUsuarios] = useState(0);
  const [pedidosRecientes, setPedidosRecientes] = useState([]);
  const [loading, setLoading] = useState(true);

  // Cambia el useEffect actual de tu AdminDashboard por este:
useEffect(() => {
  const obtenerDatosDashboard = async () => {
    try {
      setLoading(true);
      
      // Recuperamos token de autenticación
      const token = localStorage.getItem("jwtToken") || localStorage.getItem("token");
      const headers = { Authorization: `Bearer ${token}` };

      // Hacemos las peticiones a las listas completas de la base de datos
      const [resLibros, resUsuarios, resPedidos] = await Promise.all([
        fetch("http://localhost:4002/libros", { headers }),
        fetch("http://localhost:4002/usuarios", { headers }),
        fetch("http://localhost:4002/ordenes", { headers })
      ]);

      if (resLibros.ok) {
        const listaLibros = await resLibros.json();
        const totalInput = Array.isArray(listaLibros) ? listaLibros.length : (listaLibros.libros?.length || 0);
        setTotalLibros(totalInput);
      }

      if (resUsuarios.ok) {
        const listaUsuarios = await resUsuarios.json();
        const totalUsersInput = Array.isArray(listaUsuarios) ? listaUsuarios.length : (listaUsuarios.usuarios?.length || 0);
        setTotalUsuarios(totalUsersInput);
      }

      if (resPedidos.ok) {
        const todosLosPedidos = await resPedidos.json();
        const listaPedidos = Array.isArray(todosLosPedidos) ? todosLosPedidos : (todosLosPedidos.ordenes || []);
